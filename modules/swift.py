...
class Swift(LanguageABC):
    SWIFT_BLOCK_COMMENT = re.compile(r'/\*\*.+?\*/', re.DOTALL)
    SWIFT_ONE_LINE_COMMENT = re.compile(r"//.*")
        ...
        for dir in include_dir:
            for root, _, filenames in os.walk(os.path.join(path, dir)):
                for filename in filenames:
                    if filename.endswith('.swift') and filename not in ignore:
    ...
    def comments(self, file) -> list:
        ...
        with open(file, 'r', encoding='utf-8') as f:
            content = f.read()
        block_comments = re.findall(self.SWIFT_BLOCK_COMMENT, content)
        for block_comment in block_comments:
            comment += block_comment.split('\n')
        one_line_comments = re.findall(self.SWIFT_ONE_LINE_COMMENT, content)
